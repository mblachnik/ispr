...
/*   IB3 [Aha, 1991, 1992]. Allows only 'acceptable' instances to
    ...
    public IB3InstanceSelectionModel(DistanceMeasure measure, int k, double upperInterval, double lowerInterval, RandomGenerator randomGenerator, ILossFunction loss){
    ...
    private double maxConfidence(double y, double n, double z) {
        /* Returns high end of confidence interval, given:
         y, the number of successes; n, the number of trials; and
         z, the confidence level.  Taken from Aha's IBL code.*/
        if (n == 0.0) {
            return 1;
        } else {
    ...
    private double minConfidence(double y, double n, double z) {
        /* Returns low end of confidence interval, given:
         y, the number of successes; n, the number of trials; and
         z, the confidence level.  Taken from Aha's IBL code.
         (Only difference from MaxConfidence is the '- val' instead of '+ val'.*/
        if (n == 0.0) {
            return 0;
        } else {
            double frequency = y / n;
